...
if "posts" not in st.session\_state:
st.session\_state.posts = \[]

# Hàm sinh caption bằng GPT

def generate\_caption(product\_name, keywords, platform):
prompt = f"""
Bạn là chuyên gia nội dung sáng tạo cho thương hiệu gốm thủ công cao cấp.

Hãy viết một **bài viết marketing dài khoảng 100–150 từ** phù hợp đăng trên {platform}, để giới thiệu sản phẩm **{product\_name}**, sử dụng tinh tế các từ khóa: {keywords}.
...
product\_name = st.text\_input("Tên sản phẩm")
keywords = st.text\_input("Từ khóa", "gốm, thủ công, mộc mạc, decor")
...
if st.session\_state.posts:
df = pd.DataFrame(st.session\_state.posts)
for i, row in df.iterrows():
with st.expander(f"{row\['platform']} | {row\['caption']\[:30]}..."):
df.at\[i, 'likes'] = st.number\_input(f"❤️ Likes #{i}", value=int(row\['likes']), key=f"likes\_{i}")
df.at\[i, 'comments'] = st.number\_input(f"💬 Comments #{i}", value=int(row\['comments']), key=f"comments\_{i}")
df.at\[i, 'shares'] = st.number\_input(f"🔁 Shares #{i}", value=int(row\['shares']), key=f"shares\_{i}")
df.at\[i, 'reach'] = st.number\_input(f"📣 Reach #{i}", value=int(row\['reach']), key=f"reach\_{i}")
...
{df\[\['platform','caption','likes','comments','shares','reach']].to\_string(index=False)}
...
Hãy dự đoán hiệu quả của bài viết dưới đây trên nền tảng {platform\_forecast} nếu được đăng vào lúc {post\_time\_forecast.strftime("%H:%M %d/%m/%Y")}.
...
4. 💡 Gợi ý cách viết lại nếu cần
   ...
   st.error(f"⚠️ Không gọi được GPT: {e}")  sửa code để có các access kia
