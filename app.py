...
with tab1:
    ...
    if mode == "📅 Tự động đúng giờ":
        st.session_state["post_date_once"] = st.date_input("📅 Ngày đăng", st.session_state["post_date_once"], key="post_date_once")
        st.session_state["post_time_once"] = st.time_input("⏰ Giờ đăng", st.session_state["post_time_once"], key="post_time_once")

    elif mode == "🤖 Tự động đăng đa dạng mỗi ngày":
        st.session_state["start_date_loop"] = st.date_input("📅 Ngày bắt đầu", st.session_state["start_date_loop"], key="start_date_loop")
        st.session_state["end_date_loop"] = st.date_input("📅 Ngày kết thúc", st.session_state["end_date_loop"], key="end_date_loop")
        st.session_state["post_time_loop"] = st.time_input("⏰ Giờ đăng mỗi ngày", st.session_state["post_time_loop"], key="post_time_loop")
